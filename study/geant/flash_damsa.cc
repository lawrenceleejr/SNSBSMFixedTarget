...
// radius at crossing):
//   plane 0: z = 0.24 m, r < 0.1 m  (DAMSA chamber entrance, behind plug)
//   plane 1: z = 0.55 m, r < 0.1 m  (DAMSA calorimeter face)
...
namespace {
constexpr double kPlaneZ[3] = {24. * cm, 55. * cm, 1500. * cm};
constexpr double kPlaneR[3] = {10. * cm, 10. * cm, 100. * cm};
...
class DetectorConstruction : public G4VUserDetectorConstruction {
 public:
  G4VPhysicalVolume* Construct() override {
    ...
    auto* dumpS = new G4Tubs("Dump", 0., 15. * cm, 5. * cm, 0., 360. * deg);
    auto* dumpL = new G4LogicalVolume(dumpS, tungsten, "Dump");
    new G4PVPlacement(nullptr, {0., 0., 5. * cm}, dumpL, "Dump", worldL,
                      false, 0, true);
    // ~30 X0 tungsten photon plug between dump and decay chamber
    auto* plugS = new G4Tubs("Plug", 0., 12. * cm, 5. * cm, 0., 360. * deg);
    auto* plugL = new G4LogicalVolume(plugS, tungsten, "Plug");
    new G4PVPlacement(nullptr, {0., 0., 17. * cm}, plugL, "Plug", worldL,
                      false, 0, true);
    return worldP;
  }
};
...
class FlashSteppingAction : public G4UserSteppingAction {
 public:
  FlashSteppingAction() {
    ...
    name << "flash_damsa_hits_t" << G4Threading::G4GetThreadId() << ".csv";
    fOut.open(name.str());
